...
class Analysis(object):
    ...
    def __init__(self, db_source, population_name, 
    ...
    def get_individual_status_list_by_generation(self, status, generation):
        status_dict = database_calls.db_get_organisms_status(self.cur, 
            self.starting_time, self.population_name, status, [generation])
        status_list = list(status_dict[generation].values())
        return status_list

    def get_individual_genome_list_by_generation(self, generation):
        genome_dict = database_calls.db_get_organisms_genome(self.cur, 
    ...
    def analyze_individual_status_by_generation(self, csv_output, status, 
                                                status_analysis, 
    ...
    def analyze_individual_genomes_by_generation(self, csv_output, 
        ...
        for generation in generation_list:
            ...
            status_row = [str(generation)] + \
                         [str(genome_result) 
                          for genome_result in genome_list]
            if aggregate_functions != None:
                for key in list(aggregate_functions.keys()):
        ...
        for generation in generation_list:
            ...
            analyzed_genome_list = \
                [genome_analysis(self.get_status_group_genome_by_generation(status, 
                    target_status, generation)) 
                 for target_status in stats]
            status_row = [str(generation)] + \
                         [str(status_result) 
                          for status_result in analyzed_genome_list]
            if aggregate_functions != None:
                for key in list(aggregate_functions.keys()):
